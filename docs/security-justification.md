...
| **Assume breach** | Private endpoints supported for Storage, Key Vault, and OpenAI. Azure Monitor logging on all ingestion events. No secrets in code or environment variables (Managed Identity throughout). Collector certificate generated and stored in Key Vault — rotation is a single Key Vault operation with no code changes. |
...
- **Certificate management**: Collector certificate is auto-generated by Key Vault at deploy time and fetched at runtime via `DefaultAzureCredential` — no local cert files
...
- **Function key**: All endpoints require `x-functions-key` header (Azure Functions built-in auth)
- **mTLS**: Optional client certificate validation via `X-ARR-ClientCert` header — enabled by setting `ALLOWED_CERT_THUMBPRINTS`; skipped when not configured (dev mode)
...
- Certificate thumbprint used (when mTLS is enabled)
